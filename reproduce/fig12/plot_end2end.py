...
def collect_data():
    ...
    for baseline in ['bitblas', 'vllm', 'qfactory', 'llama_cpp']:
        data[f'{baseline}:4'] = data[baseline][:6]
        data[f'{baseline}:2'] = data[baseline][6:]
    return data
...
def plot(data):
    ...
    mask = list(range(0, len(models)))[:-1]

    width = 0.2  # Bar width
    ax1.bar(x[mask] - 1.5 * width, speedup_llama_cpp_4bit, width, label='llama.cpp', color='tab:red')
    ax1.bar(x[mask] - 0.5 * width, speedup_vllm_4bit, width, label='Marlin', color='#9355B0')
    ax1.bar(x[mask] + 0.5 * width, speedup_bitblas_4bit, width, label='BitBLAS', color='#78D3AC')
    ax1.bar(x[mask] + 1.5 * width, speedup_qfactory_4bit, width, label='QFactory', color='#EF7F51')
    for i, v in zip(x[mask], speedup_qfactory_4bit):
        ax1.text(i, v + 0.05, f"({data['qfactory:4'][i]:.1f})", ha='center', fontsize=6, rotation=30)
    
    ax1.text(len(models) - 1, 0.1, 'OOM', ha='center', fontsize=7, color='tab:red', weight='bold')
    ...
    plt.savefig(f'end2end_a100.pdf')
